...
<template>
  <c-card>
    ...
    <div style="opacity: 0.8">
      {{ t('tools.mime-types.mimeToExtensionDesc') }}
    </div>
    <c-select
    ...
    <div v-if="extensionsFound.length > 0">
      {{ t('tools.mime-types.extensionsOfMime', [selectedMimeType]) }}
  ...
  <c-card>
    ...
    <div style="opacity: 0.8">
      {{ t('tools.mime-types.extensionToMimeDesc') }}
    </div>
    <c-select
      ...
      :placeholder="t('tools.mime-types.extensionSelectPlaceholder')"
    />

    <div v-if="selectedExtension">
      {{ t('tools.mime-types.mimeOfExtension', [selectedExtension]) }}
      <div style="margin-top: 10px">
  ...
  <div>
    <n-table>
      <thead>
        <tr>
          <th>{{ t('tools.mime-types.tableMimeType') }}</th>
