...
const { copy: copyFileBase64 } = useCopy({
  source: fileBase64,
  text: computed(() => t('tools.base64-file-converter.copied')),
});
...
<template>
  ...
  <c-card :title="$t('tools.base64-file-converter.fileToBase64Title')">
    <c-file-upload :title="$t('tools.base64-file-converter.uploadPrompt')" @file-upload="onUpload" />
    <c-input-text
      :value="fileBase64"
      multiline
      readonly
    ...
    <div flex justify-center>
      <c-button @click="copyFileBase64()">
        {{ $t('tools.base64-file-converter.copyButton') }}
      </c-button>
    </div>
  </c-card>
</template>
